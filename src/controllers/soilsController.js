...
const createSoil = async function (req, res) {
    try {
        ...
        const { soilType,soilElements, soilWeather } = data;
        if (!dataValidation.isValidState(soilType)) return res.status(400).send({ status: false, message: "soilType should be string" });
        if (!dataValidation.isValidState(Object.values(soilElements)[0])) return res.status(400).send({ status: false, message: "soilElements should be string" });
        ...
        const soilData = {
            soilType: soilType, soilElements:soilElements, soilWeather: soilWeather 
        };
